...
class EvalCallback():
    ...
    # ----------------------------------------------------------#
    #   Process multi-modal input, read RGB and NIR images and generate prediction results
    # ----------------------------------------------------------#
    def get_map_txt(self, image_id, image_rgb, image_nir, class_names, map_out_path):
        ...
        # Convert RGB and NIR images
        image_rgb = cvtColor(image_rgb)
        image_nir = cvtColor(image_nir)

        # Resize and process RGB and NIR separately
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for annotation_line in tqdm(self.val_lines):
                ...
                image_rgb = Image.open(line[0])
                image_nir = Image.open(line[1])  # Assume NIR image path is second item

                # Get predicted txt
                self.get_map_txt(image_id, image_rgb, image_nir, self.class_names, self.map_out_path)
