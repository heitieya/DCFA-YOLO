...
def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9, roll_out=False):
    ...
    # Calculate the distance from each ground truth box to each anchor point's top-left and bottom-right corners, then take the minimum
    # Ensure the ground truth box is near the anchor point, enclosing the anchor point
    if roll_out:
        ...
        # Ground truth box's top-left and bottom-right coordinates 
        lt, rb      = gt_bboxes.view(-1, 1, 4).chunk(2, 2)  
        # Calculate the distance from each ground truth box to each anchor point's top-left and bottom-right corners
...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    ...
    if fg_mask.max() > 1:  
        ...
        # If an anchor is assigned to predict multiple ground truth boxes, first calculate the most overlapping ground truth box for this anchor
...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        # Get the maximum number of boxes in the ground truth
        self.n_max_boxes    = gt_bboxes.size(1)
        # If self.n_max_boxes is greater than self.roll_out_thr, then roll out
...
class Loss:
    ...
    def preprocess(self, targets, batch_size, scale_tensor):
        if targets.shape[0] == 0:
            ...
            # Loop through the batch and assign values
    ...
    def bbox_decode(self, anchor_points, pred_dist):
        ...
        # Then decode to get the predicted bounding box
        return dist2bbox(pred_dist, anchor_points, xywh=False)

    def __call__(self, preds, batch):
        # Get the device being used
        device  = preds[1].device
        # Loss for box, cls, dfl
        ...
        # Get anchors and stride tensors
        anchor_points, stride_tensor = make_anchors(feats, self.stride, 0.5)

        # Put a batch of things into a matrix
        # 0 is which image
        # 1 is class
        # 2: are box coordinates
        targets                 = torch.cat((batch[:, 0].view(-1, 1), batch[:, 1].view(-1, 1), batch[:, 2:]), 1)
        # First process the input gt, padding to the maximum number and scaling the box coordinates
        # bs, max_boxes_num, 5
        ...
        # Find which boxes are positive and which are padding
        # bs, max_boxes_num
