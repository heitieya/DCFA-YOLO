...
def voc_ap(rec, prec, class_name):
    path = './map_out'
    ...
    with open(RESULTS_FILES_PATH + f"/AP_data_{class_name}.txt", 'w') as f:
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out'):
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            ap, mrec, mprec = voc_ap(rec[:], prec[:], class_name)
            ...
            if draw_plot:
                ...
                fig = plt.gcf()
                plt.title('AP ' + class_name + 'class: ' + text)
                plt.xlabel('Recall')
...
#     print("Ground Truth JSON Path:", GT_JSON_PATH)

#     try:
#        cocoGt = COCO(GT_JSON_PATH)
#     except Exception as e:
#        print("Error loading COCO ground truth:", e)
...
#     # 获取 Precision 和 Recall 数据
#     precision = cocoEval.eval['precision']
#     recall = cocoEval.eval['recall']

#     # 计算 Precision 和 Recall，IoU=0.5 时的指标
#     # precision 和 recall 的维度是：[IoU_thresholds, area_range, max_dets, num_categories, num_thresholds]
#     # 获取 IoU = 0.5 时，所有类别的 Precision 和 Recall

#     precision_at_0_5 = precision[0, :, :, 0, -1]  # IoU=0.5, area=all, maxDet=100, for all categories
#     recall_at_0_5 = recall[0, :, 0, -1]            # IoU=0.5, area=all, maxDet=100, for all categories

#     # 计算所有类别的平均 Precision 和 Recall
#     avg_precision = precision_at_0_5.mean()
#     avg_recall = recall_at_0_5.mean()
...
#     print("Average Precision (IoU=0.5): ", avg_precision)
#     print("Average Recall (IoU=0.5): ", avg_recall)
#     print("F1 Score: ", f1_score)

#     # 返回 COCO eval 的所有统计数据（mAP等）
#     return cocoEval.stats, avg_precision, avg_recall, f1_score
...
def get_coco_map(class_names, path):
    ...
    # 提取每个类别的 Precision, Recall, F1
    results_per_category = []
    results_per_category_iou50 = []
    results_per_category_iou75 = []

    for idx, class_name in enumerate(class_names):
        # 获取 IoU = 0.50:0.95 时的 Precision 和 Recall
        precision = precisions[:, :, idx, 0, -1]  # IoU=0.50:0.95, area=all, maxDet=100, for class idx
        recall = recalls[:, idx, 0, -1]  # IoU=0.50:0.95, area=all, maxDet=100, for class idx

        # 获取 IoU = 0.50 时的 Precision 和 Recall
        precision_50 = precisions[0, :, idx, 0, -1]  # IoU=0.50
        recall_50 = recalls[0, idx, 0, -1]  # IoU=0.50

        # 获取 IoU = 0.75 时的 Precision 和 Recall
        precision_75 = precisions[5, :, idx, 0, -1]  # IoU=0.75
        recall_75 = recalls[5, idx, 0, -1]  # IoU=0.75

        # 计算 F1 Score: F1 = 2 * (Precision * Recall) / (Precision + Recall)
        f1_score = calculate_f1(np.mean(precision), np.mean(recall))
        f1_score_50 = calculate_f1(np.mean(precision_50), np.mean(recall_50))
        f1_score_75 = calculate_f1(np.mean(precision_75), np.mean(recall_75))

        # 保留小数点后三位
        precision = np.round(np.mean(precision), 3)
        recall = np.round(np.mean(recall), 3)
        f1_score = np.round(np.mean(f1_score), 3)

        precision_50 = np.round(np.mean(precision_50), 3)
        recall_50 = np.round(np.mean(recall_50), 3)
        f1_score_50 = np.round(np.mean(f1_score_50), 3)

        precision_75 = np.round(np.mean(precision_75), 3)
        recall_75 = np.round(np.mean(recall_75), 3)
        f1_score_75 = np.round(np.mean(f1_score_75), 3)

        # 添加到结果列表
        results_per_category.append([class_name, precision, recall, f1_score])
        results_per_category_iou50.append([class_name, precision_50, recall_50, f1_score_50])
        results_per_category_iou75.append([class_name, precision_75, recall_75, f1_score_75])

    # 打印每个类别的结果 (IoU=0.50:0.95)
